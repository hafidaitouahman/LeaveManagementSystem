...
export class CreateQuotaComponent implements OnInit {
  ...
  save() {
    this.quotaService.createLeaveQuota(this.quota).subscribe(
      (data: Quota) => {
        console.log(data);
  ...
  // quota: Quota = new Quota();
  // submitted = false;
  // constructor(private fb: FormBuilder, private quotaService: QuotaService,private userService: UserService,  private router: Router,public modal: NgbActiveModal) {}
  // ngOnInit() {
  // }
  ...
  // save() {
  //   this.quotaService
  //   .createLeaveQuota(this.quota).subscribe(data => {
  //     console.log(data)
  //     this.quota = new Quota();
  //     this.modal.close(this.quota); // Ferme la boîte de dialogue
  //     this.gotoList();
